...
from gme.estimate.Specification import Specification
from gme.estimate.DiagnosticsLog import DiagnosticsLog
from gme.estimate.combine_sector_results import combine_sector_results
from gme.estimate._slice_data_for_estimation import _slice_data_for_estimation
from gme.estimate._ppml_estimation_and_diagnostics import _estimate_ppml
import pickle as pickle
from gme.estimate.format_regression_table import format_regression_table
from gme.estimate.SlimResults import SlimResults
...
class EstimationModel(object):
    def __init__(self,
        ...
        Args:
            ...
            omit_fixed_effect: (optional)
                The fixed effect category from which to drop a fixed effect to avoid collinearity. The entry should be a
                subset of the list supplied for fixed_effects. In each case, the last fixed effect is dropped. If not
                specified, the colinearity diagnostics will identify a column to drop on its own.
    ...
    def estimate(self):
        ...
        meta_data = data_object._meta_data
